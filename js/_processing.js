...
function Player() {
    ...
    this.move = function() {
        ...
        for (var i = 1; i < frames + 1; i++) {
            var that = this;
            animationsArray.push(setTimeout(drawStep, animationSpeed * animationCounter, i));
            animationCounter++;
    ...
    this.display = function() {
        ...
        if (currentLevel <= 20) {
            ...
            image(playerImg2, -that.diameter / 2, -that.diameter / 2, that.diameter, that.diameter);
        }
        pop();
    }
};
...
function LoadLevel1() {
    ...
    $("#tipModal1 .modal-body").html('<h3>You need to connect the "Walk" block to the "On Start" block to make the cat walk.</h3>');
...
function LoadLevel2() {
    if (currentLevel != 2) {
        $("#instructionsModal .modal-body").html("<h3>Program the cat to walk to the red circle</h3>");
        $("#instructionsModal").modal("show");
        currentLevel = 2;
        tipToShow = 1;
        totalNumTips = 3;
...
function LoadLevel4() {
    if (currentLevel != 4) {
        $("#instructionsModal .modal-body").html('<h3>Use "Jump" to avoid the purple circles and reach the red circle.</h3>'); //.append('<svg width="20" height="20"><circle cx="10" cy="10" r="10" fill="purple" /></svg>');
        $("#instructionsModal").modal("show");
        currentLevel = 4;
        tipToShow = 1;
        totalNumTips = 3;
    ...
    $("#tipModal1 .modal-body").html('<h3>Try using the "Walk" and "Jump" block to walk and jump over the purple circles.</h3>');
...
function LoadLevel6() {
    if (currentLevel != 6) {
        $("#instructionsModal .modal-body").html("<h3>Avoid the purple circles to reach the red circle.</h3>");
        $("#instructionsModal").modal("show");
        currentLevel = 6;
        tipToShow = 1;
        totalNumTips = 3;
...
function LoadLevel21() {
    if (currentLevel != 21) {
        $("#instructionsModal .modal-body").html('<h3>Can you move the bogo to the green circle?</h3>');
        $("#instructionsModal").modal("show");
        currentLevel = 21;
        tipToShow = 1;
        totalNumTips = 3;
...
function LoadLevel24() {
    if (currentLevel != 24) {
        $("#instructionsModal .modal-body").html('<h3>Get the bogo to the green circle.</h3>');
        $("#instructionsModal").modal("show");
        currentLevel = 24;
        tipToShow = 1;
        totalNumTips = 2;
...
function LoadLevel99() {
    if (currentLevel != 99) {
        $("#instructionsModal .modal-body").html("<h3>Choose the right path to reach the red circle. You must do this USING 9 BLOCKS or less.</h3>");
        $("#instructionsModal").modal("show");
        ...
        if (walkDist == obstacles[i].x) { //Is there an obstacle at ground level
            ...
            for (var j = 0; j < obstacles.length; j++) {
                ...
                if ((Math.abs(heightDistDouble - obstacles[j].y) < 5) && (walkDist == obstacles[j].x)) { //Is there a stacked obstacle. Use < to account for height variations on jump animations
                    console.log("j: " + j);
                    console.log("Obstacle X: " + obstacles[j].x + " Y: " + obstacles[j].y);
                    console.log("Player X: " + player.x + " Y: " + player.y);
                    console.log("walkDist: " + walkDist + " heightDistDouble: " + heightDistDouble);
                    return true;
                }
            }
...
function displayBlockOutput(block, text) {
    ...
    console.log("Block position from displayBlockOutput: $iframeOffset.left: " + $iframeOffset.left + " X: " + position.x + ", Y: " + position.y + ", width: " + blockWidth + ", toolboxWidth: " + toolboxWidth);
    $output.text(text);
    $output.css({
